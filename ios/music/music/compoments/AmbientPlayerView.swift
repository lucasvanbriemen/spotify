...
struct AmbientPlayerView: View {
    ...
    var body: some View {
        ...
        GeometryReader { geo in
            ...
            ZStack {
                AmbientBackground(colors: manager.artworkPalette, dimmed: sleepActive)
                    .ignoresSafeArea()

                if sleepActive {
                    // Extra scrim so even a bright cover stays gentle at night.
...
/// Slowly drifting mesh gradient built from the album artwork's palette. When
/// `dimmed`, every stop is darkened — light covers especially — so the
/// nighttime view never glares.
struct AmbientBackground: View {
    ...
    private func palette(for colors: [Color]) -> [Color] {
        ...
        guard dimmed else { return base }
        return base.map { darken($0.resolve(in: environment)) }
    }

    private func darken(_ c: Color.Resolved) -> Color {
        let r = Double(c.red), g = Double(c.green), b = Double(c.blue)
        let luminance = 0.2126 * r + 0.7152 * g + 0.0722 * b
        // Pull light colors down hard, dark ones a little, so the mood stays dim.
        let factor = luminance > 0.5 ? 0.18 : 0.34
